...
FILES <- list.files("Data/","xIRTQrs.tsv",full.names=T)
FILES <- FILES[grep(pattern="Ch",x=FILES)]
NAMES <- basename(gsub(x=FILES,pattern="_xIRTQrs.tsv",replacement=""))
...
fwrite(Df,"Data/ChPsyQrs.tsv",quote=F,col.names=T,row.names=F,sep="\t",na="NA")

gDf <- Df %>% inner_join(gID) %>% select(FID,IID,everything()) %>% select(-PrID)
fwrite(gDf,"Data/ChPsyGenQrs.tsv",quote=F,col.names=T,row.names=F,sep="\t",na="NA")

# fwrite(Df,"Data/FamPsyQrs.tsv",quote=F,col.names=T,row.names=F,sep="\t",na="NA")
...
fwrite(Df2,"ChPsyQrsAsr.tsv",quote=F,col.names=T,row.names=F,sep="\t",na="NA")

gDf <- Df2 %>% inner_join(gID) %>% select(FID,IID,everything()) %>% select(-PrID,-FamID)
fwrite(gDf,"Data/ChPsyGenQrsAsr.tsv",quote=F,col.names=T,row.names=F,sep="\t",na="NA")
...
fwrite(Df2,"Data/FamPsyQrs_PreImp.tsv",quote=F,col.names=T,row.names=F,sep="\t",na="NA")
...
fwrite(Df3,"Data/FamPsyQrs_CovPreImp.tsv",quote=F,col.names=T,row.names=F,sep="\t",na="NA")
